...
class DeltaXYWHBBoxCoder(BaseBBoxCoder):
    ...
    def __init__(self,
        ...
        self.means = torch.tensor(target_means,dtype=torch.float32).cuda().unsqueeze(0)
        self.stds = torch.tensor(target_stds,dtype=torch.float32).cuda().unsqueeze(0)
...
def __bbox2delta(proposals, gt, means=(0., 0., 0., 0.), stds=(1., 1., 1., 1.)):
...
def bbox2delta(proposals, gt, means=(0., 0., 0., 0.), stds=(1., 1., 1., 1.)):
    ...
    proposals = proposals.float()
    gt = gt.float()
    pxy = (proposals[..., :2] + proposals[..., 2:]) * 0.5
    pwh = proposals[..., 2:] - proposals[..., :2]
    pwh.clamp_(min=1e-3)
    ...
    whr.clamp_(min=1e-4)
    dwh = torch.log(whr)
    deltas = torch.cat([dxy, dwh], dim=-1)
