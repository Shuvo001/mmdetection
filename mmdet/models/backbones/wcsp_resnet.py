...
from .yolo_bricks import CSPResLayer
from .yolobase_backbone import YOLOBaseBackbone
...
class WCSPResNet(YOLOBaseBackbone):
    ...
    def build_stem_layer(self) -> nn.Module:
        if self.deep_stem:
            return self.build_deep_stem_layer(self.input_channels,self.arch_setting[0][0],
                activation_fn="LeakyReLU")
        else:
            return self.build_normal_stem_layer()

    def build_deep_stem_layer(self,in_channels,stem_channels,activation_fn="LeakyReLU"):
        return build_stem(self.deep_stem_mode,in_channels,stem_channels,activation_fn=activation_fn)

    def build_normal_stem_layer(self) -> nn.Module:
        """Build a stem layer."""
