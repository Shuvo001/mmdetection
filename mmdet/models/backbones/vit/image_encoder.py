...
class ImageEncoderViT(nn.Module):
    def __init__(
        ...
        stem_channels=64,
        deep_stem_mode="MultiBranchStemSA12X", #convs, MultiBranchStem12X, MultiBranchStemS12X, MultiBranchStemSL12X
        ...
        self.stem = self._make_stem_layer(in_chans, stem_channels,activation_fn=stem_activation_fn)
        ...
        self.scale = nn.ModuleList([
            nn.ConvTranspose2d(embed_dim,out_chans,4,4),
            nn.ConvTranspose2d(embed_dim,out_chans,2,2),
            nn.Conv2d(embed_dim,out_chans,1,1),
            nn.Conv2d(out_chans,out_chans,2,2),
        ])
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        for i,x in enumerate(outs):
            outs[i] = self.scale[i](x)

        return outs

    def _make_stem_layer(self, in_channels, stem_channels,activation_fn="ReLU"):
        stem = self._make_deep_stem_layer(in_channels,stem_channels,activation_fn=activation_fn)
        return stem
