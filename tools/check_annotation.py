...
import numpy as np
from iotoolkit.pascal_voc_toolkit import PascalVOCData,write_voc_xml,read_voc_xml
...
def load_annotation(ann_path):
    #shape, bboxes, labels_text, difficult, truncated, probs = read_voc_xml(ann_path)
    data = read_voc_xml(ann_path)
    return data
...
def main():
    ...
    if save_path is None:
        save_path = osp.join(work_dir,"tmp","check_annotation_"+wmlu.base_name(test_data_dir))
    ...
    sys.stdout.flush()
    label_to_text = dict(zip(range(len(classes)),classes))
    gt_save_dir = osp.join(save_path,"gt")
    ...
    for i,(full_path,img) in enumerate(reader):
        try:
            ...
            gt_labels = load_annotation(xml_path)[2]
            labels = [label_to_text[x] for x in labels] 
            ...
            shutil.copy(xml_path,gt_save_dir)
